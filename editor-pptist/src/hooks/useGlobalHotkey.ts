...
export default () => {
  ...
  const keydownListener = (e: KeyboardEvent) => {
    ...
    // Page structure is owned by the host until Local Page synchronization exists.
    if (thumbnailsFocus.value && ['ENTER','DELETE','BACKSPACE','D','V','X'].includes(key)) return
    ...
    if (false && editorAreaFocus.value && !shiftKey && !ctrlOrMetaKeyActive && !disableHotkeys.value) {
      if (key === KEYS.T) {
