...
onMounted(() => {
  ...
  document.addEventListener('banana-editor-flush', flushPendingInput)
  if (props.autoFocus) editorView.focus()
})
const flushPendingInput = () => { handleInput(); handleInput.flush() }
onUnmounted(() => {
  document.removeEventListener('banana-editor-flush', flushPendingInput)
  handleInput.cancel()
