...
from utils.auth import auth_required_enabled, create_auth_token, current_user, validate_username
...
def _with_auth_cookie(response_tuple, token: str):
    ...
    response.set_cookie(
        ...
        httponly=True,
        secure=os.getenv('AUTH_COOKIE_SECURE', '').lower() in {'1', 'true', 'yes', 'on'},
        samesite='Lax',
    )
...
def register():
    ...
    return _with_auth_cookie(success_response(payload, status_code=201), payload['token'])
...
def login():
    ...
    payload = _auth_payload(user)
    return _with_auth_cookie(success_response(payload), payload['token'])
