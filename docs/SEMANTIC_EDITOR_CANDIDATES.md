...
用户已确认在线编辑采用 PPTist，授权事项由用户联系作者处理，不作为当前技术验证的阻塞项。下文保留为前置调研记录，不表示仍在进行开放选型。下一阶段采用独立 Vue 编辑子应用与 React 宿主集成，复用统一语义模型并实现双向适配；当前提交尚未引入 PPTist、编辑路由或在线保存 API。复杂功能只有通过对象往返和 PPTX 验收后才开放。
